...
region_argo <- argo  %>% mutate(ratio_clean = corrected_fluo/tchla, ratio = fluo/tchla) %>%  group_by(code) %>% summarise_at(vars(c(ratio, ratio_clean)), c(mean, sd), na.rm = TRUE) %>% ungroup()
names(region_argo) <- c("code", "mean_ratio", "mean_ratio_clean", "sd_ratio", "sd_ratio_clean")
