...
renderPlot({ggplot()+
  ...
  geom_point(aes(x = lon, y = lat), data = filter(first_profiles, lovbio == input$lovbio), colour = "Red", size = 3)+
  coord_quickmap()+
    ggtitle(paste("lon :", unique(first_profiles$lon[first_profiles$lovbio == input$lovbio]), "; lat :", unique(first_profiles$lat[first_profiles$lovbio == input$lovbio]), "; date :", unique(first_profiles$date[first_profiles$lovbio == input$lovbio]),  sep = " "))+
  theme_bw()
})
```
...
renderPlot({ggplot()+
  ...
  geom_point(aes(x = tchla, y = chla), data = filter(merged, lovbio == input$lovbio), colour = "Red", size = 3)+
    theme_bw()
})
```
